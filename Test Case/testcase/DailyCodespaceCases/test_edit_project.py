...
def test_verify_remote_language_project(playwright: Playwright):
    ...
    try:
        ...
        page.keyboard.press("Control+Shift+E")
        if not page.get_by_role("treeitem", name="Program.cs").locator("a").is_visible():
            page.wait_for_timeout(1000)
            page.keyboard.press("Control+Shift+E")
        page.get_by_role("treeitem", name="Program.cs").locator("a").dblclick()
        page.get_by_text("Run").first.click(button="right")
        page.wait_for_timeout(1000)
        if not page.get_by_text("Go to Definition").is_visible():
            page.get_by_text("Run").first.click(button="right")
            page.wait_for_timeout(1000)
        ...
        page.get_by_role("menuitem", name="Peek", exact=True).click()
        page.wait_for_timeout(1000)
...
def test_install_extension_from_market(playwright: Playwright):
    ...
    try:
        ...
        page.get_by_role("listitem", name="C# XML Documentation Comments").get_by_title("Install this extension in all your synced Visual Studio Code - Insiders instances").click()
        page.wait_for_timeout(5000)
