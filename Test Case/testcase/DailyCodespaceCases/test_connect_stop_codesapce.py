...
def test_open_folder(new_page: Page, folderpath: string):
    ...
    new_page.get_by_role("combobox").fill(folderpath)
...
def test_connect_stress_testing(playwright : Playwright):
    ...
    try:
        ...
        if new_page.locator("button", has_text="Restart codespace").is_visible():
            new_page.locator("button", has_text="Restart codespace").click()
            new_page.wait_for_timeout(30000)
        if not new_page.get_by_role("tab", name="Remote Explorer").locator("a").is_visible():
            new_page.keyboard.press("Control+,")
            new_page.wait_for_timeout(1500)
            new_page.get_by_label("Search settings").fill("Show Performance Explorer")
            new_page.get_by_role("checkbox", name="github.codespaces.showPerformanceExplorer").check()
            new_page.wait_for_timeout(2000)
        new_page.get_by_role("tab", name="Remote Explorer").locator("a").click()
        new_page.get_by_label("Terminal 1").type("sudo apt-get update")
        new_page.keyboard.press("Enter")
        new_page.wait_for_timeout(90000)
        new_page.get_by_label("Terminal 1").type("sudo apt-get install stress-ng")
        new_page.keyboard.press("Enter")
        new_page.keyboard.press("Y")
        new_page.keyboard.press("Enter")
        new_page.wait_for_timeout(7000)
        new_page.get_by_label("Terminal 1").type("stress-ng --cpu 4")
        new_page.keyboard.press("Enter")
